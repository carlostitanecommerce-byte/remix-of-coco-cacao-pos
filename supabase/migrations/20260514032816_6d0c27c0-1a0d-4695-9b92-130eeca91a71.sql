...
BEGIN
  ...
  -- Enlazar KDS
  IF v_es_paquete THEN
    -- Para paquetes: tomar la orden KDS más reciente de la sesión (cualquier estado activo)
    SELECT ko.id INTO v_kds_order_id
      FROM public.kds_orders ko
     WHERE ko.coworking_session_id = p_session_id
       AND ko.estado IN ('pendiente','en_preparacion','listo')
     ORDER BY ko.created_at DESC
     LIMIT 1;
  ELSIF v_dv.producto_id IS NOT NULL THEN
    -- Para producto simple: incluir también órdenes ya en "listo"
    SELECT koi.id, koi.kds_order_id
     ...
     WHERE ko.coworking_session_id = p_session_id
       ...
       AND ko.estado IN ('pendiente','en_preparacion','listo')
     ORDER BY (koi.cancel_qty < koi.cantidad) DESC, koi.created_at DESC
     LIMIT 1;
  END IF;
